...
// WithFallbackModels sets backup model names used when the current model is
// temporarily unavailable or rate-limited.
func WithFallbackModels(models []string) ClientOption {
	return func(c *Config) {
		c.FallbackModels = append([]string(nil), models...)
	}
}
