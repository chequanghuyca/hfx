...
// TruncatedResponseError preserves partial assistant content when a provider
// stops at the completion-token limit. Callers can use the partial analysis in
// a short formatting-repair request instead of repeating the full market prompt.
...
func (e *TruncatedResponseError) Error() string {
	...
	return fmt.Sprintf("API returned truncated response (finish_reason=%s)", reason)
}
