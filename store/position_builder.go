...
func (pb *PositionBuilder) ProcessTrade(
	...
	if strings.HasPrefix(action, "open_") {
		...
		return pb.handleClose(traderID, exchangeID, exchangeType, symbol, side, quantity, price, fee, realizedPnL, tradeTimeMs, orderID, closeCycle)
	}
	return nil
}
...
func (pb *PositionBuilder) handleOpen(
	...
	if existing == nil {
		logger.Infof("  🐣 New open: %s %s (%.6f @ %.2f, time: %d)", symbol, side, quantity, price, tradeTimeMs)
		// Create new position
	...
	if existing.ExchangeID == "" || existing.ExchangeType == "" {
		if err := pb.positionStore.UpdatePositionExchangeInfo(existing.ID, exchangeID, exchangeType); err != nil {
			logger.Errorf("  ❌ Failed to update exchange info: %v", err)
		}
	}
...
func (pb *PositionBuilder) handleClose(
	...
	if position == nil {
		...
		logger.Infof("  ⚠️  SKIP CLOSE: No matching open position for %s %s (orderID: %s, time: %d). This usually means the opening trade was missed or skipped.", symbol, side, orderID, tradeTimeMs)
		return nil
	}
	...
	if quantity < position.Quantity-QUANTITY_TOLERANCE {
		...
		return pb.positionStore.ReducePositionQuantity(position.ID, quantity, price, fee, realizedPnL, closeCycle)
	} else {
