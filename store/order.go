...
func (s *OrderStore) InitTables() error {
	...
	if s.db.Dialector.Name() == "postgres" {
		...
		s.db.Raw(`SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'trader_orders'`).Scan(&ordersExist)
		s.db.Raw(`SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'trader_fills'`).Scan(&fillsExist)
		...
		if ordersExist > 0 && fillsExist > 0 {
			...
			for _, c := range timestampColumns {
				var dataType string
				s.db.Raw(`SELECT data_type FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = ? AND column_name = ?`, c.table, c.col).Scan(&dataType)
