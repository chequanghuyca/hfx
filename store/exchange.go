...
func (s *ExchangeStore) initTables() error {
	...
	if s.db.Dialector.Name() == "postgres" {
		...
		s.db.Raw(`SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'exchanges'`).Scan(&tableExists)
		if tableExists > 0 {
...
func (s *ExchangeStore) migrateToMultiAccount() error {
	...
	err := s.db.Model(&Exchange{}).
		Where("exchange_type = '' AND id = ?", "binance").
		Count(&count).Error
	if err != nil {
		return err
	}
	...
	err = s.db.Where("exchange_type = '' AND id = ?", "binance").
		Find(&records).Error
	if err != nil {
		return err
	}
