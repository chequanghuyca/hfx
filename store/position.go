...
type TraderPosition struct {
	...
	CloseCycle         int     `gorm:"column:close_cycle;default:0" json:"close_cycle"`
...
func (s *PositionStore) InitTables() error {
	...
	if s.isPostgres() {
		...
		s.db.Raw(`SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'trader_positions'`).Scan(&tableExists)
		if tableExists > 0 {
			...
			for _, col := range timestampColumns {
				var dataType string
				s.db.Raw(`SELECT data_type FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'trader_positions' AND column_name = ?`, col).Scan(&dataType)
...
func (s *PositionStore) ClosePosition(id int64, exitPrice float64, exitOrderID string, realizedPnL float64, fee float64, closeReason string, closeCycle int) error {
...
func (s *PositionStore) ReducePositionQuantity(id int64, reduceQty float64, exitPrice float64, addFee float64, addPnL float64, closeCycle int) error {
	var pos TraderPosition
	if err := s.db.First(&pos, id).Error; err != nil {
		return fmt.Errorf("failed to get current position: %w", err)
	}
...
func (s *PositionStore) ClosePositionFully(id int64, exitPrice float64, exitOrderID string, exitTimeMs int64, totalRealizedPnL float64, totalFee float64, closeReason string, closeCycle int) error {
	var pos TraderPosition
	if err := s.db.First(&pos, id).Error; err != nil {
...
type RecentTrade struct {
	...
	CloseCycle   int     `json:"close_cycle"`
