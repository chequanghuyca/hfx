...
func InitGormPostgres(host string, port int, user, password, dbname, sslmode, schema string) (*gorm.DB, error) {
	schema, err := normalizePostgresSchema(schema)
	if err != nil {
		return nil, err
	}

	if err := ensurePostgresSchema(host, port, user, password, dbname, sslmode, schema); err != nil {
		return nil, err
	}
...
var postgresSchemaPattern = regexp.MustCompile(`^[a-zA-Z_][a-zA-Z0-9_]*$`)

func normalizePostgresSchema(schema string) (string, error) {
	if schema == "" {
		schema = "public"
	}
...
func ensurePostgresSchema(host string, port int, user, password, dbname, sslmode, schema string) error {
	dsn := fmt.Sprintf(
	...
	if err != nil {
		return fmt.Errorf("failed to prepare PostgreSQL schema: %w", err)
	}
	defer db.Close()

	if _, err := db.Exec(`CREATE SCHEMA IF NOT EXISTS "` + schema + `"`); err != nil {
		return fmt.Errorf("failed to create PostgreSQL schema %s: %w", schema, err)
	}
	return nil
}
