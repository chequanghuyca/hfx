...
func (s *UserStore) initTables() error {
	...
	if s.db.Dialector.Name() == "postgres" {
		...
		s.db.Raw(`SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = 'users'`).Scan(&tableExists)
		...
		if tableExists > 0 {
			...
			s.db.Raw(`
				...
				WHERE schemaname = current_schema() AND tablename = 'users' AND indexdef LIKE '%email%' AND indexdef LIKE '%UNIQUE%'
			`).Scan(&indexExists)
