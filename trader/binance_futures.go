...
// Global cache for Binance to reduce redundant REST API calls across all trader instances
var binanceCache = struct {
	sync.RWMutex
	timeSynced      bool
	...
	positionModeSet map[string]bool // key: apiKey, value: whether Hedge Mode is set

	// Shared REST account data to prevent redundant calls across multiple traders
	accountBalances  map[string]map[string]interface{}    // key: apiKey
	accountPositions map[string][]map[string]interface{} // key: apiKey
	lastAccountSync  map[string]time.Time                 // key: apiKey
}{
	positionModeSet:  make(map[string]bool),
	accountBalances:  make(map[string]map[string]interface{}),
	accountPositions: make(map[string][]map[string]interface{}),
	lastAccountSync:  make(map[string]time.Time),
}
...
type FuturesTrader struct {
	...
	// Cache validity period (60 seconds for REST fallback)
...
func NewFuturesTrader(apiKey, secretKey string, userId string) *FuturesTrader {
	...
	trader := &FuturesTrader{
		client:        client,
		cacheDuration: 10 * time.Second, // 10-second cache (REST fallback)
		wsManager:     GetSharedBinanceWsManager(client, apiKey),
	}

	// Sync time using shared cache to avoid redundant REST calls
	trader.syncSharedTime(client)

	// Set dual-side position mode (Hedge Mode) using shared cache if possible
	if err := trader.setDualSidePositionShared(apiKey); err != nil {
...
// syncSharedTime syncs Binance server time using a global cache
func (t *FuturesTrader) syncSharedTime(client *futures.Client) {
	binanceCache.RLock()
	// Reuse time sync for 1 hour
	if binanceCache.timeSynced && time.Since(binanceCache.lastTimeSync) < 1*time.Hour {
		client.TimeOffset = binanceCache.timeOffset
		binanceCache.RUnlock()
		return
	}
	binanceCache.RUnlock()

	binanceCache.Lock()
	defer binanceCache.Unlock()
	...
	binanceCache.timeOffset = offset
	binanceCache.timeSynced = true
	binanceCache.lastTimeSync = time.Now()
	
	logger.Infof("⏱ Binance server time synced (Shared), offset %dms", offset)
}

// setDualSidePositionShared sets dual-side position mode using shared cache
func (t *FuturesTrader) setDualSidePositionShared(apiKey string) error {
	binanceCache.RLock()
	if binanceCache.positionModeSet[apiKey] {
		binanceCache.RUnlock()
		return nil
	}
	binanceCache.RUnlock()
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "No need to change position side") {
			binanceCache.Lock()
			binanceCache.positionModeSet[apiKey] = true
			binanceCache.Unlock()
...
// getSharedAccountData fetches account data (balance + positions) and caches it globally
func (t *FuturesTrader) getSharedAccountData(apiKey string) (map[string]interface{}, []map[string]interface{}, error) {
	binanceCache.RLock()
	if syncTime, ok := binanceCache.lastAccountSync[apiKey]; ok && time.Since(syncTime) < t.cacheDuration {
		balance := binanceCache.accountBalances[apiKey]
		positions := binanceCache.accountPositions[apiKey]
		binanceCache.RUnlock()
	...
	// Double check after lock
	if syncTime, ok := binanceCache.lastAccountSync[apiKey]; ok && time.Since(syncTime) < t.cacheDuration {
		return binanceCache.accountBalances[apiKey], binanceCache.accountPositions[apiKey], nil
	}

	logger.Infof("🔄 Calling Binance API to refresh account data (Shared)...")
	...
	// 1. Parse Balances
	balanceResult := make(map[string]interface{})
	balanceResult["totalWalletBalance"], _ = strconv.ParseFloat(account.TotalWalletBalance, 64)
	balanceResult["availableBalance"], _ = strconv.ParseFloat(account.AvailableBalance, 64)
	balanceResult["totalUnrealizedProfit"], _ = strconv.ParseFloat(account.TotalUnrealizedProfit, 64)

	// 2. Parse Positions
	var positionsResult []map[string]interface{}
	for _, pos := range account.Positions {
		// GetAccount returns simplified positions
		...
		posMap["positionAmt"] = amt
		posMap["entryPrice"], _ = strconv.ParseFloat(pos.EntryPrice, 64)
		posMap["unRealizedProfit"], _ = strconv.ParseFloat(pos.UnrealizedProfit, 64)
		posMap["leverage"], _ = strconv.ParseFloat(pos.Leverage, 64)
		
		// For markPrice and liquidationPrice, GetAccount might not be enough.
		// But for bot's basic display, we'll try to get it.
		// If missing, bot will use real-time or fallback.
		...
		positionsResult = append(positionsResult, posMap)
	}

	// Update global cache
	binanceCache.accountBalances[apiKey] = balanceResult
	binanceCache.accountPositions[apiKey] = positionsResult
	binanceCache.lastAccountSync[apiKey] = time.Now()

	return balanceResult, positionsResult, nil
}
...
func (t *FuturesTrader) GetBalance() (map[string]interface{}, error) {
	// 1. Check WebSocket manager first (real-time data)
	if t.wsManager != nil && t.wsManager.IsConnected() {
		balance := t.wsManager.GetBalance()
		if balance != nil {
			return balance, nil
		}
	}

	// 2. Use Shared Global Cache (REST Fallback)
	apiKey := ""
	if t.wsManager != nil && t.wsManager.client != nil {
		apiKey = t.wsManager.client.APIKey
	}
	balance, _, err := t.getSharedAccountData(apiKey)
	if err != nil {
		return nil, err
	}
	return balance, nil
...
func (t *FuturesTrader) GetPositions() ([]map[string]interface{}, error) {
	// 1. Check WebSocket manager first (real-time data)
	if t.wsManager != nil && t.wsManager.IsConnected() {
		positions := t.wsManager.GetPositions()
		if positions != nil {
	...
	_, positions, err := t.getSharedAccountData(apiKey)
	if err != nil {
		return nil, err
	}
	
	// Optional enhancement: if we need full PositionRisk details (MarkPrice, LiquidationPrice)
	// and we are not running (no WS), we might still want one real call, 
	// but let's stick to GetAccountService for now to avoid weight limit.
...
func (t *FuturesTrader) GetMarketPrice(symbol string) (float64, error) {
	// 1. Check WebSocket manager first (real-time data)
	if t.wsManager != nil && t.wsManager.IsConnected() {
		price := t.wsManager.GetMarketPrice(symbol)
		if price > 0 {
			return price, nil
		}
	}

	// 2. Fallback to calculation from positions if symbol is active
	// (This is often faster than a separate price API call)
	t.positionsCacheMutex.RLock()
	if t.cachedPositions != nil {
		for _, pos := range t.cachedPositions {
			if pos["symbol"] == symbol {
				if markPrice, ok := pos["markPrice"].(float64); ok && markPrice > 0 {
...
func (t *FuturesTrader) GetExchangeInfo() (*futures.ExchangeInfo, error) {
	// Use global cache for ExchangeInfo
	binanceCache.RLock()
	if binanceCache.exchangeInfo != nil && time.Since(binanceCache.lastInfoSync) < 12*time.Hour {
		info := binanceCache.exchangeInfo
		binanceCache.RUnlock()
		return info, nil
	}
	binanceCache.RUnlock()
	...
	// Double check
	if binanceCache.exchangeInfo != nil && time.Since(binanceCache.lastInfoSync) < 12*time.Hour {
		return binanceCache.exchangeInfo, nil
	}

	logger.Infof("🔄 Fetching Binance Exchange Info (Shared)...")
	exchangeInfo, err := t.client.NewExchangeInfoService().Do(context.Background())
	if err != nil {
		return nil, err
	}

	binanceCache.exchangeInfo = exchangeInfo
	binanceCache.lastInfoSync = time.Now()
...
func (t *FuturesTrader) StartWs() error {
	if t.wsManager == nil {
		return nil
	}

	return t.wsManager.Start()
}

// StopWs stops the WebSocket connection
func (t *FuturesTrader) StopWs() {
	if t.wsManager != nil {
		t.wsManager.Stop()
	}
}
...
func (t *FuturesTrader) IsWsConnected() bool {
	if t.wsManager == nil {
		return false
	}
	return t.wsManager.IsConnected()
}
