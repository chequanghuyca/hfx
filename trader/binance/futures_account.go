...
// GetBalance gets account balance (with cache and debouncing)
func (t *FuturesTrader) GetBalance() (map[string]interface{}, error) {
	// 1. First check if cache is valid (Fast Path)
	...
	// 2. Cache expired, use fetch mutex to debounce concurrent calls (Slow Path)
	// This ensures that if 10 traders call GetBalance() at the same time,
	// only one actually hits the Binance API, others wait and use the result.
	t.balanceFetchMutex.Lock()
	defer t.balanceFetchMutex.Unlock()

	// 3. Re-check cache after acquiring fetch mutex (someone might have fetched it while we waited)
	t.balanceCacheMutex.RLock()
	if t.cachedBalance != nil && time.Since(t.balanceCacheTime) < t.cacheDuration {
		t.balanceCacheMutex.RUnlock()
		return t.cachedBalance, nil
	...
	result["totalEquity"], _ = strconv.ParseFloat(account.TotalMarginBalance, 64) // Margin Balance is more accurate for Equity
