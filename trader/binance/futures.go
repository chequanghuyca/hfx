...
// Static registry to share traders for the same API key (shared account data)
var (
	traderRegistry      = make(map[string]*FuturesTrader)
	traderRegistryMutex sync.Mutex

	// Global ExchangeInfo cache (shared across all instances)
	globalExchangeInfo      *futures.ExchangeInfo
	globalExchangeInfoTime  time.Time
	globalExchangeInfoMutex sync.Mutex
)
...
type FuturesTrader struct {
	...
	balanceFetchMutex sync.Mutex // For debouncing concurrent fetches

	// Position cache
	cachedPositions     []map[string]interface{}
	positionsCacheTime  time.Time
	positionsCacheMutex sync.RWMutex
	positionsFetchMutex sync.Mutex // For debouncing concurrent fetches

	// Cache validity period (15 seconds)
	cacheDuration time.Duration
}

// GetFuturesTrader returns a shared futures trader for the given API key
// This ensures multiple bots for the same account share the same cache/connection
func GetFuturesTrader(apiKey, secretKey string, userId string) *FuturesTrader {
	if apiKey == "" {
		return NewFuturesTrader(apiKey, secretKey, userId)
	}

	traderRegistryMutex.Lock()
	defer traderRegistryMutex.Unlock()

	key := apiKey // Group by API key
	if t, exists := traderRegistry[key]; exists {
		logger.Infof("🏦 Using shared Binance trader for account (API Key: %s...)", apiKey[:8])
		return t
	}

	logger.Infof("🏦 Creating new shared Binance trader for account (API Key: %s...)", apiKey[:8])
	t := NewFuturesTrader(apiKey, secretKey, userId)
	traderRegistry[key] = t
	return t
}

// NewFuturesTrader creates futures trader (internal use or standalone)
func NewFuturesTrader(apiKey, secretKey string, userId string) *FuturesTrader {
	...
	if apiKey != "" {
		if err := trader.setDualSidePosition(); err != nil {
			logger.Infof("⚠️ Failed to set dual-side position mode: %v (ignore if already in dual-side mode)", err)
		}
	}
...
// GetExchangeInfoWithCache gets exchange info with global caching
func (t *FuturesTrader) GetExchangeInfoWithCache() (*futures.ExchangeInfo, error) {
	globalExchangeInfoMutex.Lock()
	defer globalExchangeInfoMutex.Unlock()

	// Cache for 1 hour as exchange rules change rarely
	if globalExchangeInfo != nil && time.Since(globalExchangeInfoTime) < 1*time.Hour {
		return globalExchangeInfo, nil
	}

	logger.Infof("🔄 Fetching global Binance ExchangeInfo (cache expired or first run)...")
	info, err := t.client.NewExchangeInfoService().Do(context.Background())
	if err != nil {
		return nil, err
	}

	globalExchangeInfo = info
	globalExchangeInfoTime = time.Now()
	return info, nil
}
