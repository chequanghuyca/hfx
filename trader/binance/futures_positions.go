...
// GetPositions gets all positions (with cache and debouncing)
func (t *FuturesTrader) GetPositions() ([]map[string]interface{}, error) {
	...
	// 2. Cache expired, use fetch mutex to debounce (Slow Path)
	t.positionsFetchMutex.Lock()
	defer t.positionsFetchMutex.Unlock()

	// 3. Re-check cache after acquiring fetch mutex
	t.positionsCacheMutex.RLock()
	if t.cachedPositions != nil && time.Since(t.positionsCacheTime) < t.cacheDuration {
		t.positionsCacheMutex.RUnlock()
		return t.cachedPositions, nil
...
// GetSymbolPrecision gets the quantity precision for a trading pair (uses global cache)
func (t *FuturesTrader) GetSymbolPrecision(symbol string) (int, error) {
	exchangeInfo, err := t.GetExchangeInfoWithCache()
	if err != nil {
...
// GetSymbolPricePrecision gets the price precision for a trading pair (uses global cache)
