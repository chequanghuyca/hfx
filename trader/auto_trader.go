...
func NewAutoTrader(config AutoTraderConfig, st *store.Store, userID string) (*AutoTrader, error) {
	...
	// OPEN_ROUTER_API_KEY enables a process-wide fixed model override.
	resolvedModel := config.CustomModelName
	mcpClient, resolvedModel = llm.NewClient(aiModel, apiKey, customURL, resolvedModel)
	if _, forced := llm.ForcedOpenRouterConfig(); forced {
		aiModel = llm.OpenRouterModelID
		logger.Infof("🤖 [%s] Using fixed OpenRouter model: %s", config.Name, resolvedModel)
	} else {
	...
	case "binance":
		...
		trader = binance.GetFuturesTrader(config.BinanceAPIKey, config.BinanceSecretKey, userID)
	default:
