...
func (tm *TraderManager) LoadUserTradersFromStore(st *store.Store, userID string) error {
	...
	for _, traderCfg := range traders {
		...
		if forced, ok := llm.ForcedOpenRouterConfig(); ok {
			aiModelCfg = &store.AIModel{
				ID:              llm.OpenRouterModelID,
				UserID:          userID,
				Name:            "OpenRouter fixed model",
				Provider:        llm.OpenRouterProvider,
				Enabled:         true,
				APIKey:          crypto.EncryptedString(forced.APIKey),
				CustomAPIURL:    forced.BaseURL,
				CustomModelName: forced.Model,
			}
		} else if aiModelCfg == nil {
...
func (tm *TraderManager) addTraderFromStore(traderCfg *store.Trader, aiModelCfg *store.AIModel, exchangeCfg *store.Exchange, st *store.Store) error {
	...
	// Load strategy config (fallback to active/default if not specified)
	...
	if strategyID != "" {
		strategy, err = st.Strategy().Get(traderCfg.UserID, strategyID)
		if err != nil {
			logger.Infof("⚠️ Failed to load specified strategy %s for trader %s, trying active/default: %v", strategyID, traderCfg.Name, err)
			strategy, err = st.Strategy().GetActiveOrDefault(traderCfg.UserID)
		}
	} else {
		logger.Infof("📝 Trader %s has no strategy ID, loading active/default fallback", traderCfg.Name)
		strategy, err = st.Strategy().GetActiveOrDefault(traderCfg.UserID)
	}

	if err != nil || strategy == nil {
		return fmt.Errorf("trader %s has no strategy configured and no default available: %w", traderCfg.Name, err)
	}
	...
	store.ApplySmallAccountBalancedMode(strategyConfig)
	logger.Infof("🛡️ Trader %s runtime strategy mode: balanced small account (mixed sources with static fallback)", traderCfg.Name)
	logger.Infof("✓ Trader %s using strategy: %s (%s)", traderCfg.Name, strategy.Name, strategy.ID)
	...
	if exchangeCfg.ExchangeType == "binance" {
	...
	default:
		// For other providers (gemini, grok, openai, claude, kimi, etc.), use CustomAPIKey
