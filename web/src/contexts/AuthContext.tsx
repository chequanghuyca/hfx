...
const WHITELIST_EMAILS = [
  ...
  'huy.che0503@gmail.com',
  'votuanthanh7a3@gmail.com',
]
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    getSystemConfig()
      .then((config) => {
        // A fresh database/schema has no users. Discard tokens from a previous
        // installation so the UI does not send stale identities to this backend.
  ...
  const login = async (email: string, password: string) => {
    ...
    if (!isWhitelisted(email)) {
      ...
      return {
        success: false,
        message: 'This email is not authorized to access this platform.',
      }
    }
