...
export interface AccountInfo {
  ...
  unrealized_profit: number // Unrealized PnL (official value from exchange API)
...
export interface CreateExchangeRequest {
  ...
  account_name: string // User-defined account name
  enabled: boolean
  is_running?: boolean // Optional field from backend
  api_key?: string
  secret_key?: string
  passphrase?: string
  testnet?: boolean
...
export interface CreateTraderRequest {
  ...
  initial_balance?: number // Optional: auto-fetched by backend on create, can be manually updated on edit
  scan_interval_minutes?: number
  is_cross_margin?: boolean
  show_in_competition?: boolean // Whether to show in competition arena
  // These fields are kept for backward compatibility, new version uses strategy config
...
export interface Strategy {
  ...
  is_active: boolean
  is_default: boolean
  is_public: boolean // Whether publicly visible in strategy market
  config_visible: boolean // Whether config parameters are publicly visible
...
export interface StrategyStats {
  ...
  top_performers?: StrategyPerformer[] // Top performers ranking
}

// Strategy user performance ranking
export interface StrategyPerformer {
  user_id: string
  user_name: string // Anonymized username
  total_pnl_pct: number // Total return percentage
  total_pnl: number // Total profit amount
  win_rate: number // Win rate
  trade_count: number // Number of trades
  using_since: string // Start time of using strategy
  rank: number // Rank
}
...
export interface IndicatorConfig {
  ...
  // Price ranking data (price change ranking)
...
export interface KlineConfig {
  ...
  // Support selecting multiple timeframes
...
export interface RiskControlConfig {
  ...
  max_risk_per_trade_pct: number // Max equity loss at stop, e.g. 0.008 = 0.8% (CODE ENFORCED)
}
