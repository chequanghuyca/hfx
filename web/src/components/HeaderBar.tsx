...
export default function HeaderBar({
  ...
  // const [languageDropdownOpen, setLanguageDropdownOpen] = useState(false) // Commented out - language dropdown not shown
  const [userDropdownOpen, setUserDropdownOpen] = useState(false)
  // const dropdownRef = useRef<HTMLDivElement>(null) // Language dropdown removed
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      // Language dropdown removed - commented out
      // if (
      //   dropdownRef.current &&
  ...
  return (
    <nav className="fixed top-0 w-full z-50 header-bar">
      <div className="flex items-center justify-between h-16 max-w-[1920px] mx-auto px-4 md:px-24">
        ...
        >
          <span className="text-lg font-bold text-nofx-gold">HFX</span>
        </div>
            ...
            {isLoggedIn && user ? (
              <div className="flex items-center gap-3">
                ...
                <div className="relative" ref={userDropdownRef}>
                  ...
                  {userDropdownOpen && (
                    <div className="absolute right-0 top-full mt-2 w-auto rounded-lg shadow-lg overflow-hidden z-50 bg-nofx-bg-lighter border border-nofx-gold/20">
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-x-0 z-40 md:hidden bg-[#0a0a0f] border-t border-zinc-800/50"
            style={{ top: '64px' }}
          >
            <motion.div
              ...
              className="flex flex-col h-[calc(100vh-64px)] overflow-y-auto px-6 py-8 bg-gradient-to-b from-[#0a0a0f] to-[#05050a]"
            >
              {/* Navigation Links */}
                ...
                {(() => {
                  ...
                  return navTabs.map((tab, i) => (
                    <motion.button
                      ...
                      className={`text-lg font-black tracking-tight text-left flex items-center gap-3
              ...
              <div className="mt-auto space-y-8 w-full">
                {/* Account / Lang */}
                <div className="grid grid-cols-2 gap-4 mb-16 w-full">
                  {/* Auth Actions */}
                  {isLoggedIn && user ? (
                    <button
                      onClick={() => {
                      ...
                      className="h-12 w-full bg-red-500/10 border border-red-500/20 text-red-500 rounded-lg font-bold text-sm hover:bg-red-500/20 transition-colors"
                    >
                      {t('exitLogin', language)}
                    </button>
                  ) : (
                    ...
                    currentPage !== 'register' && (
                      <a
                        href="/login"
                        className="flex h-12 w-full items-center justify-center bg-nofx-gold text-black rounded-lg font-bold text-sm hover:bg-yellow-400 transition-colors"
                      >
                        {t('signIn', language)}
                      </a>
                    )
