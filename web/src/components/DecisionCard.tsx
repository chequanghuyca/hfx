...
const ACTION_CONFIG: Record<
  string,
  { color: string; bg1: string; bg2: string; label: string }
> = {
  open_long: {
    color: '#0ECB81',
    bg1: 'rgba(103, 152, 133, 0.15)',
    bg2: 'rgba(103, 152, 133, 0.05)',
    label: 'LONG',
  },
  open_short: {
    color: '#F6465D',
    bg1: 'rgba(246, 70, 93, 0.15)',
    bg2: 'rgba(246, 70, 93, 0.05)',
    label: 'SHORT',
  },
  close_long: {
    color: '#F0B90B',
    bg1: 'rgba(240, 185, 11, 0.15)',
    bg2: 'rgba(240, 185, 11, 0.05)',
    label: 'CLOSE',
  },
  close_short: {
    color: '#F0B90B',
  ...
  hold: {
    color: '#848E9C',
    bg1: 'rgba(132, 142, 156, 0.15)',
    bg2: 'rgba(132, 142, 156, 0.05)',
    label: 'HOLD',
  },
  wait: {
    color: '#848E9C',
...
// Single Action Card Component - Memoized
const ActionCard = memo(function ActionCard({
  ...
  return (
    <div
      ...
      style={{
        background: `linear-gradient(135deg, ${config.bg1} 50%, ${config.bg2} 40%)`,
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span
            className="font-mono font-bold text-sm md:text-base cursor-pointer transition-colors duration-200"
            style={{ color: '#EAECEF' }}
          ...
          <span
            className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider md:text-sm"
            style={{
      ...
      {isOpen && (
        <div className="grid grid-cols-4 gap-3 mt-3 pt-3 border-t border-gray-500/50">
          {/* Entry Price */}
          <div className="text-center">
            ...
            <div
              className="font-mono font-semibold md:text-sm text-xs"
              style={{ color: '#EAECEF' }}
            >
      ...
      {isOpen && action.stop_loss && action.take_profit && action.price && (
        <div className="mt-3 pt-3 flex items-center justify-between border-t border-gray-500/50">
...
// DecisionCard - Memoized to prevent re-renders when decisions haven't changed
export const DecisionCard = memo(function DecisionCard({
  ...
  return (
    <div
      className="rounded-xl p-2 md:p-5 transition-colors duration-300"
      style={{
          ...
          <div>
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {showSystemPrompt ? 'Collapse' : 'Expand'}
                </span>
              </div>
            </button>
            {showSystemPrompt && (
