...
export const AdvancedChart = memo(function AdvancedChart({
  ...
  // Track the last key we fitted content for. Only fit again if this changes.
  // Using refs for fitted state - persist across re-renders but reset across page loads
  const fittedKeyRef = useRef<string>('')
  // Track if user has manually interacted with chart (zoom/pan)
  const userHasInteractedRef = useRef<boolean>(false)
  const [tooltipData, setTooltipData] = useState<any>(null)
  const tooltipRef = useRef<HTMLDivElement>(null)
  const prevKlineDataRef = useRef<any[] | null>(null) // Track previous kline data for comparison

  // Reset previously fitted key when symbol/interval/exchange changes
  // Only reset if it's a real configuration change, not a re-mount
  const prevConfigRef = useRef<string>('')
  useEffect(() => {
    const currentConfig = `${symbol}-${interval}-${exchange}`
    // Only reset if configuration actually changed (not just component re-mount)
    if (prevConfigRef.current && prevConfigRef.current !== currentConfig) {
      console.log(
        '[ChartDebug] Config change detected:',
  ...
  const { data: klineData } = useQuery({
    queryKey: ['chart-klines', symbol, interval, exchange],
  ...
  const { data: ordersData } = useQuery({
    queryKey: ['chart-orders', traderID, symbol],
    queryFn: () => fetchOrders(traderID!, symbol),
  ...
  // 3. Open Orders Query (Price Lines)
  const { data: openOrdersData } = useQuery({
    queryKey: ['chart-open-orders', traderID, symbol],
    queryFn: () => fetchOpenOrders(traderID!, symbol),
    enabled: !!traderID && isTraderRunning,
    refetchInterval: 60000,
    placeholderData: keepPreviousData,
    notifyOnChangeProps: ['data'],
  ...
  useEffect(() => {
    ...
    // Track user interaction with chart (zoom/pan) to prevent auto fitContent
    // Use a flag to skip the first emission (which happens on initial load)
    ...
    chart.timeScale().subscribeVisibleTimeRangeChange(() => {
      ...
      // User has manually interacted with the chart
  ...
  // Update Chart Data when K-line data changes
  useEffect(() => {
    if (!candlestickSeriesRef.current || !klineData) return

    // Optimization: Skip update if data hasn't meaningfully changed
    const prevData = prevKlineDataRef.current
    if (prevData && prevData.length > 0 && klineData.length > 0) {
      const lastCandle = klineData[klineData.length - 1]
      const prevLastCandle = prevData[prevData.length - 1]

      // Skip if same length and last candle time/price unchanged
      if (
        prevData.length === klineData.length &&
        prevLastCandle?.time === lastCandle?.time &&
        prevLastCandle?.close === lastCandle?.close &&
        prevLastCandle?.high === lastCandle?.high &&
        prevLastCandle?.low === lastCandle?.low
      ) {
        return // No meaningful change, skip expensive setData
      }
    }
    prevKlineDataRef.current = klineData

    // 1. Set K-line Data
    candlestickSeriesRef.current.setData(klineData)

    // Store volume/quoteVolume stats for tooltip
    ...
    // 2. Market Stats - only update if values actually changed
    if (klineData.length > 0) {
      const latestKline = klineData[klineData.length - 1]
      const prevKline =
        klineData.length > 1 ? klineData[klineData.length - 2] : latestKline

      const priceChange = latestKline.close - prevKline.close
      const priceChangePercent =
        prevKline.close !== 0 ? (priceChange / prevKline.close) * 100 : 0

      setMarketStats((prev) => {
        // Only update if values actually changed to prevent re-renders
        if (
          prev &&
          prev.price === latestKline.close &&
          prev.high === latestKline.high &&
          prev.low === latestKline.low
        ) {
          return prev
        }
        return {
    ...
    // Initial fit content - Only if we haven't fitted for this specific config yet
    // AND user has not manually interacted with the chart
    const currentKey = `${symbol}-${interval}-${exchange}`

    if (
      chartRef.current &&
      fittedKeyRef.current !== currentKey &&
      !userHasInteractedRef.current
    ) {
      console.log('[ChartDebug] Fitting content for:', currentKey)
      chartRef.current.timeScale().fitContent()
      fittedKeyRef.current = currentKey
    }

    setLoading(false)
  }, [klineData, indicators, symbol, interval, exchange]) // Include dependencies for key generation

  // Update Orders Markers when data changes
  useEffect(() => {
    if (
    ...
    try {
      // RE-IMPLEMENTING MARKER LOGIC IN A CLEANER WAY
      // We need klineData to match times.
      if (!klineData || klineData.length === 0) return

      const klineTimes = klineData.map((k: any) => k.time as number)
      const klineMinTime = klineTimes[0]
      const klineMaxTime = klineTimes[klineTimes.length - 1]

      // Binary search for candle time
      const findCandleTime = (orderTime: number): number | null => {
        if (orderTime < klineMinTime || orderTime > klineMaxTime) return null
        let left = 0,
          right = klineTimes.length - 1
        while (left < right) {
      ...
      currentMarkersDataRef.current = newMarkers

      const markersToShow = showOrderMarkers ? newMarkers : []
      if (!seriesMarkersRef.current) {
      ...
      console.error('[AdvancedChart] Failed to update markers:', err)
    }
  }, [ordersData, klineData, showOrderMarkers])

  // Update Open Orders (Price Lines)
  useEffect(() => {
    if (!candlestickSeriesRef.current || !traderID) return
