      ...
      <div className="flex items-center gap-4 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <CustomSelect
            ...
            onChange={(val) => setSelectedSymbol(String(val))}
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-[10px]" style={{ color: '#5E6673' }}>
                    Qty: {pos.quantity.toFixed(4)} · Margin: $
...
export function BacktestPage() {
  ...
  const { data: runsResp } = useQuery({
    ...
    queryFn: () => api.getBacktestRuns({ limit: 100, offset: 0 }),
    staleTime: 5000,
  ...
  const { data: status } = useQuery<BacktestStatusPayload>({
    queryKey: ['bt-status', selectedRunId],
    queryFn: () => api.getBacktestStatus(selectedRunId!),
    enabled: !!selectedRunId,
    staleTime: 2000,
    refetchInterval: 3000,
  })

  const { data: equity } = useQuery<BacktestEquityPoint[]>({
    queryKey: ['bt-equity', selectedRunId],
    queryFn: () => api.getBacktestEquity(selectedRunId!, '1m', 2000),
    enabled: !!selectedRunId,
    staleTime: 5000,
    refetchInterval: 10000,
  })

  const { data: trades } = useQuery<BacktestTradeEvent[]>({
    queryKey: ['bt-trades', selectedRunId],
    queryFn: () => api.getBacktestTrades(selectedRunId!, 500),
    enabled: !!selectedRunId,
    staleTime: 5000,
    refetchInterval: 10000,
  })
  ...
  const { data: metrics } = useQuery<BacktestMetrics>({
    queryKey: ['bt-metrics', selectedRunId],
    queryFn: () => api.getBacktestMetrics(selectedRunId!),
    enabled: !!selectedRunId,
    staleTime: 10000,
  ...
  const { data: decisions } = useQuery<DecisionRecord[]>({
    queryKey: ['bt-decisions', selectedRunId],
    queryFn: () => api.getBacktestDecisions(selectedRunId!, 30),
    enabled: !!selectedRunId,
    staleTime: 5000,
    refetchInterval: 10000,
  })
                    ...
                    >
                      <div>
                        ...
                        <CustomSelect
                          ...
                          onChange={(val) =>
                            handleFormChange('aiModelId', String(val))
                          }
                          placeholder={tr('form.selectAiModel')}
                          options={[
                            { value: '', label: tr('form.selectAiModel') },
                            ...(aiModels?.map((m) => ({
                              value: m.id,
                              label: `${m.name} (${m.provider}) ${!m.enabled ? '⚠️' : ''}`,
                            })) || []),
                          ]}
                          className="w-full"
                        />
                      ...
                      <div>
                        ...
                        <CustomSelect
                          ...
                          onChange={(val) =>
                            handleFormChange('strategyId', String(val))
                          }
                          placeholder="No saved strategy"
                          options={[
                            {
                              value: '',
                              label: 'No saved strategy',
                            },
                            ...(strategies?.map((s) => ({
                              value: s.id,
                              label: `${s.name} ${s.is_active ? '✓' : ''} ${s.is_default ? '⭐' : ''}`,
                            })) || []),
                          ]}
                          className="w-full"
                        />
                          ...
                          >
                            ...
                            {strategyHasDynamicCoins && (
                              <div
                                ...
                                style={{ color: '#F0B90B' }}
                              >
                                ⚡ Clear the symbols field below to use
                                strategy's dynamic coins
                              </div>
                            )}
                          </div>
                        )}
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <CustomSelect
                            ...
                            onChange={(val) =>
                              handleFormChange('decisionTf', String(val))
                            }
                            options={formState.timeframes.map((tf) => ({
                              value: tf,
                              label: tf,
                            }))}
                            className="w-full"
              ...
              <>
                ...
                <div className="binance-card p-4">
                  ...
                  {status?.positions && status.positions.length > 0 && (
                    <PositionsDisplay positions={status.positions} />
                  )}
                </div>
