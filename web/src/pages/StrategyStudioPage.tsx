...
import type { AIModel, Strategy, StrategyConfig } from '../types'

const API_BASE = import.meta.env.VITE_API_BASE || ''

const StrategyStudioSkeleton = () => {
  return (
    <SkeletonTheme baseColor="#1E2329" highlightColor="#2B3139">
      <div className="h-[calc(100vh-64px)] flex flex-col bg-nofx-bg overflow-hidden animate-pulse">
        {/* Header Skeleton */}
        <div className="flex-shrink-0 px-4 py-3 border-b border-nofx-gold/20 bg-nofx-bg/60 backdrop-blur-md">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="hidden md:flex w-48 flex-col border-r border-nofx-gold/20 bg-nofx-bg/30 p-2 space-y-4">
          ...
          <div className="flex-1 border-r border-nofx-gold/20 overflow-hidden flex flex-col">
          ...
          <div className="hidden lg:flex w-80 xl:w-96 flex-col bg-nofx-bg/30">
            <div className="flex border-b border-white/5">
              <div className="flex-1 h-10 border-b-2 border-transparent"></div>
              <div className="flex-1 h-10 border-b-2 border-transparent"></div>
            </div>
...
export function StrategyStudioPage() {
  ...
  const handleDeleteStrategy = async (id: string) => {
    ...
    const confirmed = await confirmToast('Delete this strategy?', {
    ...
    },
    {
      ...
      content: editingConfig && (
        <textarea
          ...
          className="w-full mt-2 min-h-72 sm:min-h-80 px-3 py-2 rounded-lg resize-y font-mono text-xs"
          style={{
            background: '#0B0E11',
  ...
  return (
    <DeepVoidBackground className="h-[calc(100vh-64px)] flex flex-col bg-nofx-bg relative overflow-hidden">
      {/* Header */}
      <div className="flex-shrink-0 px-3 sm:px-4 py-2 sm:py-3 border-b border-nofx-gold/20 bg-nofx-bg/60 backdrop-blur-md z-10">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <div className="p-1.5 sm:p-2 rounded-lg bg-gradient-to-br from-nofx-gold to-yellow-500 flex-shrink-0">
              <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-black" />
            </div>
            <div className="min-w-0">
              <h1 className="text-base sm:text-lg font-bold text-nofx-text truncate">
                Strategy Studio
              </h1>
              <p className="text-[10px] sm:text-xs text-nofx-text-muted hidden sm:block">
                Configure and test trading strategies
              </p>
            </div>
          </div>
          {error && (
            <div className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[10px] sm:text-xs bg-nofx-danger/10 text-nofx-danger flex-shrink-0">
      ...
      {/* Mobile Panel Tabs */}
      <div className="md:hidden flex-shrink-0 flex border-b border-nofx-gold/20 bg-nofx-bg/80 backdrop-blur-sm">
        <button
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div
          className={`${
            activeMobilePanel === 'list' ? 'flex' : 'hidden'
          } md:flex w-full md:w-48 flex-shrink-0 border-r border-nofx-gold/20 overflow-y-auto bg-nofx-bg/30 backdrop-blur-sm z-10 flex-col`}
        >
          <div className="p-2">
        ...
        <div
          ...
          } md:flex flex-1 min-w-0 overflow-y-auto border-r border-nofx-gold/20 flex-col`}
        >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 sm:mb-4 gap-2">
                <div className="flex-1 min-w-0">
                  <input
                    ...
                    disabled={selectedStrategy.is_default}
                    className="text-base sm:text-lg font-bold bg-transparent border-none outline-none w-full text-nofx-text placeholder-nofx-text-muted"
                  />
                  <input
                    type="text"
                    ...
                    className="text-[10px] sm:text-xs bg-transparent border-none outline-none w-full text-nofx-text-muted placeholder-nofx-text-muted/50 mt-0.5 sm:mt-1"
                  />
                  {hasChanges && (
                    <span className="text-[10px] sm:text-xs text-nofx-gold">
                      ● Unsaved
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  {!selectedStrategy.is_active && (
                    <button
                      ...
                      className="flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded text-[10px] sm:text-xs transition-colors bg-nofx-success/10 border border-nofx-success/30 text-nofx-success hover:bg-nofx-success/20"
                    >
                  ...
                  {!selectedStrategy.is_default && (
                    <button
                      ...
                      className={`flex items-center gap-1 px-4 sm:px-5 py-1.5 rounded text-[10px] sm:text-xs font-medium transition-colors disabled:opacity-50
        ...
        <div
          className={`${
            activeMobilePanel === 'preview' ? 'flex' : 'hidden'
          } md:flex w-full md:w-80 lg:w-[420px] flex-shrink-0 flex-col overflow-hidden`}
        >
          {/* Tabs */}
            ...
            {activeRightTab === 'prompt' ? (
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <CustomSelect
                    ...
                    onChange={(val) => setSelectedVariant(String(val))}
                    options={[
                  ...
                  >
                    ...
                    {promptPreview ? 'Refresh Prompt' : 'Load Prompt'}
                  </button>
                </div>
                    ...
                    <div className="p-2 rounded-lg bg-nofx-bg border border-nofx-gold/20">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[10px] sm:text-xs">
                        {Object.entries(promptPreview.config_summary || {}).map(
                          ([key, value]) => (
                            <div key={key}>
                              <div className="text-nofx-text-muted truncate">
                                {key.replace(/_/g, ' ')}
                              </div>
                              <div className="text-nofx-text truncate">
                                {String(value)}
                              </div>
                            </div>
                          )
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-nofx-text-muted">Test Note</p>
                </div>
