  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-sm text-nofx-text font-medium">
          {msg.ai_model_name}
        </span>
        ...
        <span className="text-xs text-nofx-gold font-medium">
          {msg.decision?.confidence || msg.confidence}%
        </span>
        {open ? (
          <ChevronUp size={14} className="text-nofx-text-muted" />
        ) : (
          <ChevronDown size={14} className="text-nofx-text-muted" />
        )}
      </div>
    ...
    <div className="bg-nofx-bg-lighter/40 backdrop-blur-md rounded-xl p-4 border border-nofx-gold/20 hover:border-nofx-gold/50 transition-all shadow-lg hover:shadow-[0_0_20px_rgba(240,185,11,0.1)]">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <span className="text-nofx-text font-semibold block">
              {vote.ai_model_name}
            </span>
            {vote.symbol && (
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
        <div className="flex justify-between">
          <span className="text-nofx-text-muted">Leverage</span>
          <span className="text-nofx-text font-semibold">
            {vote.leverage || '-'}x
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-nofx-text-muted">Position</span>
          <span className="text-nofx-text font-semibold">
            {vote.position_pct
        ...
        <div className="flex justify-between">
          <span className="text-nofx-text-muted">SL</span>
        ...
        <div className="flex justify-between">
          <span className="text-nofx-text-muted">TP</span>
      ...
      {vote.reasoning && (
        <p className="mt-3 text-xs text-nofx-text-muted leading-relaxed line-clamp-2 border-t border-nofx-gold/10 pt-2">
          {vote.reasoning}
        </p>
      )}
    </div>
      ...
      <div className="bg-nofx-bg-lighter/90 backdrop-blur-xl rounded-xl w-full max-w-md p-6 border border-nofx-gold/30 shadow-2xl shadow-nofx-gold/10">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={onClose}>
            <X size={20} className="text-nofx-text-muted" />
          </button>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <CustomSelect
              value={maxRounds}
              onChange={(val) => setMaxRounds(Number(val))}
          ...
          <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                ...
                <CustomSelect
                  ...
                  onChange={(val) => {
                    ...
                    up[i].personality = String(val) as DebatePersonality
                    setParticipants(up)
                  }}
                  options={Object.entries(PERS).map(([k, v]) => ({
                    value: k,
                    label: `${v.emoji} ${language === 'zh' ? v.name : v.nameEn}`,
                  }))}
                  className="w-28 !py-1 !px-2 !text-xs !bg-transparent border-0 shadow-none"
                />
                {/* AI model selector */}
                <CustomSelect
                  value={p.ai_model_id}
                  ...
                  className="w-32 !py-1 !px-2 !text-xs !bg-transparent border-0 shadow-none"
                />
                <button
                  onClick={() =>
                    setParticipants(participants.filter((_, j) => j !== i))
                  }
                  className="text-nofx-danger hover:text-red-300"
                >
                  <X size={12} />
                </button>
              </div>
            ...
            <button
              onClick={addP}
              className="px-2 py-1 text-xs text-nofx-gold hover:bg-nofx-gold/10 rounded"
            >
              + {t('addAI', language)}
            </button>
          </div>
        ...
        <div className="flex gap-2 mt-4">
          <button
            onClick={onClose}
            className="flex-1 py-2 rounded-lg bg-nofx-bg border border-nofx-gold/20 text-nofx-text text-sm hover:bg-nofx-bg-lighter transition-colors"
          >
            {t('cancel', language)}
          </button>
          <button
...
export function DebateArenaPage() {
  ...
  const { data: debates } = useQuery<DebateSession[]>({
    queryKey: ['debates'],
    queryFn: api.getDebates,
    staleTime: 5000,
  ...
  const { data: traders } = useQuery<TraderInfo[]>({
    queryKey: ['traders'],
    queryFn: api.getTraders,
    staleTime: 10000,
  })

  const { data: detail } = useQuery<DebateSessionWithDetails>({
    queryKey: ['debate', selectedId],
    queryFn: () => api.getDebate(selectedId!),
    enabled: !!selectedId,
    staleTime: 3000,
    refetchInterval: 5000,
  })
  const mutateDetail = () =>
    queryClient.invalidateQueries({ queryKey: ['debate', selectedId] })
  ...
  return (
    <DeepVoidBackground className="h-full flex overflow-hidden relative">
      ...
      <div className="w-56 flex-shrink-0 bg-nofx-bg/80 backdrop-blur-md border-r border-nofx-gold/20 flex flex-col z-10">
        ...
        {/* Debate List */}
        <div className="px-2 py-1 text-xs text-nofx-text-muted font-semibold">
          ...
          {debates?.map((d) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-sm text-nofx-text truncate flex-1">
                  {d.name}
                </span>
              </div>
          ...
          <div className="flex-1 overflow-y-auto px-2 space-y-2">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-nofx-text font-medium truncate">
                        {tr.trader_name}
                      </div>
            ...
            {traders
              ...
              .map((tr) => (
                <div
                  key={tr.trader_id}
                  className="p-2 rounded-lg bg-nofx-bg-lighter opacity-50"
                >
                  <div className="flex items-center gap-2">
        ...
        {detail ? (
          <>
            ...
            <div className="px-3 py-2 border-b border-nofx-gold/20 bg-nofx-bg/60 backdrop-blur-md flex items-center gap-3 flex-shrink-0 shadow-sm">
              ...
              />
              <span className="font-bold text-nofx-text truncate">
                {detail.name}
              </span>
              <span className="text-nofx-gold font-semibold">
                {detail.symbol}
              </span>
              {strategyName && (
              ...
              <div className="flex gap-1 ml-2">
                {participants.map((p) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className="flex items-center gap-1 px-1 py-0.5 rounded bg-nofx-bg-lighter text-xs"
                    >
              ...
              <div className="p-3 border-t border-nofx-gold/20 bg-gradient-to-r from-nofx-gold/10 via-nofx-bg-lighter/50 to-orange-500/10 backdrop-blur-md flex items-center gap-4 flex-shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  {decision ? (
                    <>
                      {decision.symbol && (
                        <span className="text-nofx-gold font-bold mr-1">
                          {decision.symbol}
                        </span>
                      )}
                      <span
                ...
                {decision && (
                  <div className="flex items-center gap-4 text-sm">
                    <span>
                      ...
                      <span className="text-nofx-gold font-bold">
                    ...
                    {(decision.leverage ?? 0) > 0 && (
                      <span>
                        ...
                        <span className="text-nofx-text font-bold">
                          {decision.leverage}x
                        </span>
                      </span>
                    )}
      ...
      {execId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="bg-nofx-bg-lighter/90 backdrop-blur-xl rounded-xl w-full max-w-sm p-6 border border-nofx-gold/30 shadow-2xl shadow-nofx-gold/10">
            ...
            <CustomSelect
              value={traderId}
              onChange={(val) => setTraderId(String(val))}
              placeholder={`${t('selectTrader', language)}...`}
              options={[
                ...(traders
                  ?.filter((tr) => tr.is_running)
                  .map((tr) => ({
                    value: tr.trader_id,
                    label: `✅ ${tr.trader_name}`,
                  })) || []),
                ...(traders
                  ?.filter((tr) => !tr.is_running)
                  .map((tr) => ({
                    value: tr.trader_id,
                    label: `⏹ ${tr.trader_name} (${t('offline', language)})`,
                  })) || []),
              ]}
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2 rounded-lg bg-nofx-bg text-nofx-text text-sm hover:bg-nofx-bg-light transition-colors"
              >
                {t('cancel', language)}
              </button>
              <button
