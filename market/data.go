...
func GetWithExchange(symbol, exchange string) (*Data, error) {
	...
	lsRatio, _ := getLongShortRatio(symbol)
...
// getLongShortRatio retrieves the global long/short account ratio from Binance
func getLongShortRatio(symbol string) (float64, error) {
	// Binance endpoint for global long/short account ratio
	...
	ratio, _ := strconv.ParseFloat(results[0].LongShortRatio, 64)
	return ratio, nil
}
...
func Format(data *Data) string {
	...
	sb.WriteString(fmt.Sprintf("Funding Rate: %.2e | Long/Short Ratio: %.2f\n\n", data.FundingRate, data.LongShortRatio))
