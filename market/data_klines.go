...
// KlineCache is a simple TTL cache for k-line data
...
var (
	klineCacheMap sync.Map // map[string]*KlineCache
)
...
func getKlinesFromCoinAnk(symbol, interval, exchange string, limit int) ([]Kline, error) {
	// Check cache
	cacheKey := fmt.Sprintf("coinank:%s:%s:%s", symbol, interval, exchange)
	if cached, ok := klineCacheMap.Load(cacheKey); ok {
		cache := cached.(*KlineCache)
		ttl := 30 * time.Second // 30s for intraday
		if parseTimeframeToMinutes(interval) >= 60 {
			ttl = 5 * time.Minute // 5m for hourly/daily
		}
		if time.Since(cache.UpdatedAt) < ttl {
			return cache.Klines, nil
		}
	}
	...
	// Update cache
	klineCacheMap.Store(cacheKey, &KlineCache{
		Klines:    klines,
		UpdatedAt: time.Now(),
	})
...
func getKlinesFromHyperliquid(symbol, interval string, limit int) ([]Kline, error) {
	// Check cache
	cacheKey := fmt.Sprintf("hyperliquid:%s:%s", symbol, interval)
	if cached, ok := klineCacheMap.Load(cacheKey); ok {
		cache := cached.(*KlineCache)
		ttl := 30 * time.Second
		if parseTimeframeToMinutes(interval) >= 60 {
