...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		s.route(api, "POST", "/verify-otp", "Verify login OTP", s.handleVerifyOTP)
		s.route(api, "POST", "/complete-registration", "Verify initial setup OTP", s.handleCompleteRegistration)
...
Use this to enable/disable an exchange or update API credentials. The "id" field is required to identify which exchange to update.`,
			...
			// Telegram bot configuration removed (requires missing store methods)
...
StrategyConfig fields:
  ...
  risk_control.max_risk_per_trade_pct: maximum equity loss at stop (recommended 0.005-0.01)
