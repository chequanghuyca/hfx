...
func (s *Server) runRealAITest(userID, modelID, systemPrompt, userPrompt string) (string, error) {
	if _, forced := llm.ForcedOpenRouterConfig(); forced {
		aiClient, _ := llm.NewClient("", "", "", "")
		response, err := aiClient.CallWithMessages(systemPrompt, userPrompt)
		if err != nil {
			return "", fmt.Errorf("OpenRouter AI API call failed: %w", err)
		}
		return response, nil
	}
	...
	aiClient, _ := llm.NewClient(provider, apiKey, model.CustomAPIURL, model.CustomModelName)
