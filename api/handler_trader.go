...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	// Generate trader ID (stable across sessions)
	exchangeIDShort := req.ExchangeID
	if len(exchangeIDShort) > 8 {
		exchangeIDShort = exchangeIDShort[:8]
	}
	// Use a combination of exchange, model and Name (which is user-unique for that user)
	// Replace spaces/special chars in name for a clean ID
	...
	traderID := fmt.Sprintf("%s_%s_%s", exchangeIDShort, req.AIModelID, cleanName)
	...
	if err != nil {
		...
		if fullCfg != nil && fullCfg.Trader != nil {
			...
			if fullCfg.Strategy == nil {
				c.JSON(http.StatusBadRequest, gin.H{"error": "Trader has no strategy configured and no system default was found. Please create a strategy in Strategy Studio."})
				return
			}
			// Check AI model
