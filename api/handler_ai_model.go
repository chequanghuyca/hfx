...
func (s *Server) handleGetModelConfigs(c *gin.Context) {
	...
	if forced, ok := llm.ForcedOpenRouterConfig(); ok {
		openRouterModel := SafeModelConfig{
			ID:              llm.OpenRouterModelID,
			Name:            "OpenRouter (fixed environment model)",
		...
		if len(models) == 0 {
			c.JSON(http.StatusOK, []SafeModelConfig{openRouterModel})
			return
		}

		safeModels := make([]SafeModelConfig, 0, len(models)+1)
		safeModels = append(safeModels, openRouterModel)
...
func (s *Server) handleUpdateModelConfigs(c *gin.Context) {
	...
	for modelID, modelData := range req.Models {
		...
		allTraders, _ := s.store.Trader().List(userID)
		for _, t := range allTraders {
			if t.AIModelID == modelID {
